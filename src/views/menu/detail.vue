          ...
          <el-timeline>
            ...
            >
              ...
              <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <div v-if="step.duration" class="text-amber-500 mb-3 text-sm">
                  预计时间：{{ step.duration }}分钟
                </div>
                <div v-if="step.actions?.length" class="space-y-2">
                  <div
...
import type { GET_MENU_LIST_RES } from "@/api/menu/types/index";
...
const menuInfo = reactive<GET_MENU_LIST_RES>({
  _id: "",
  name: "",
  ingredients: [],
  servings: 0,
...
const getTotalTime = computed(() => {
  return menuInfo.steps.reduce(
    (total, step) => total + Number(step.duration),
    0
  );
});
...
function handleEdit() {
  addUpdateRef.value?.reset({ ...menuInfo });
